...
class EvalCliIntegrationTests(unittest.TestCase):
    ...
    def test_end_to_end_generates_paired_reports(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result["runtime"]["task_model_requested"], "runtime-default")
            self.assertEqual(result["runtime"]["task_model_reported"], "runtime-default")
            self.assertEqual(result["runtime"]["judge_adapter"], "codex")
            self.assertEqual(result["runtime"]["judge_adapter_version"], "fake-codex 1.0")
            self.assertEqual(result["runtime"]["judge_model_requested"], "runtime-default")
            self.assertEqual(result["runtime"]["judge_model_reported"], "runtime-default")
            ...
            self.assertEqual(reproduce[reproduce.index("--task-adapter") + 1], "codex")
            self.assertEqual(reproduce[reproduce.index("--judge-adapter") + 1], "codex")
